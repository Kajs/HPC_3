...
void matmult_gpu2_compute(int m,int n,int k,double *d_A,double *d_B,double *d_C) {
  ...
  if(x < n && y < m) {
    ...
    d_C[i] = 0.0;
    for (l = 0; l < k; l++) {
      d_C[i] += d_A[y*k + l] * d_B[l*n + x];
    }
  }
}

__global__
void matmult_gpu3_compute(int m,int n,int k,double *d_A,double *d_B,double *d_C) {
  int i, l; 
...
void matmult_gpu3(int m,int n,int k,double *h_A,double *h_B,double *h_C) {
  ...
  int grid_x = n/(factor*2) + 1;
  int grid_y = m/(factor*1) + 1;
  ...
  dim3 dimGrid(grid_x, grid_y, 1);
  dim3 dimBlock(block_x, block_y, 1);
  printf("gpu3: running %d\n",k);
  matmult_gpu3_compute<<<dimGrid, dimBlock>>>(m, n, k, d_A, d_B, d_C);
  cudaDeviceSynchronize();
